...
model{
  ...
  for (n in 1:N_Comb){
    ...
    logit_theta[n] <- (beta_theta[1] * logmu_s[n] + beta_theta[2] * logsigma_s[n] + alpha_theta);
  }
...
generated quantities{
  ...
  real<lower=0, upper=1> theta[N_Comb];
  ...
  for (n in 1:N_Comb){
    ...
    theta[n] <- inv_logit(beta_theta[1] * log(mu_s[n]) + beta_theta[2] * log(sigma_s[n]) + alpha_theta);
  }
}
